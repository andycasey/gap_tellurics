...
__all__ = [
    'gaussian_profile', 'voigt_profile', 'lorentzian_profile',
    'measure_line'
]


def gaussian_profile(x, parameters, continuum):
    ...
    pos, fwhm, amp = parameters

    # Is the continuum a callable function?
...
def lorentzian_profile(x, parameters, continuum):
    """Evaluates a Lorentzian absorption profile across the
    `x` values with a given local continuum.

    L(x) = (1/pi) * (width/((x - pos)**2 + width**2))
    ...
    Notes
    -----
    Although `amp` (e.g. the third number in `parameters`)
    is not utilised, `parameters` should have a length of 3
    to be consistent with the `gaussian_profile` and
    `voigt_profile` functions.
    """

    pos, width, amp = parameters
...
def measure_line(spectrum, rest_wavelength, fwhm_guess, function='gaussian',
    local_continuum=True, use_central_weighting=False, window=10, order=2,
    detection_sigma=1.0, max_iter=4, shape_guess=1.0, **kwargs):
    """Iteratively fits an absorption profile to the pixels in a given spectrum at the
    ...
    try:
        p1, cov_p, infodict, mesg, ier = optimize.leastsq(errfunc, p0.copy()[0], args=args, full_output=True)
        ...
        if local_continuum:
            ...
            for iteration in xrange(max_iter):
                ...
                for i, outlier_group in enumerate(outlier_groups):
                    ...
                    try: rest_wavelength_p1 = optimize.leastsq(errfunc, rest_wavelength_p0.copy()[0], args=args, full_output=True)
                    except: continue
                    else:
        ...
        #idx = np.searchsorted(spectrum.disp, [rest_wavelength - tol_profile, rest_wavelength + tol_profile])
